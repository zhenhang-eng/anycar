#!/bin/bash
# Asymmetric guard hysteresis: entry margin 1.0 / dwell 5, warm return
# immediate (hard floor restored). Nine episodes on the same grid harness.
...
for scen in "nominal|1.6|0,0,0,0,0,0" "high|2.8|0,0,0,0,0,0" "recovery|2.8|0,0.3,0.3,0,0,0"; do
  ...
  id="ghist_m1.0_d5whr_${name}_s${seed}"
  dir="$ROOT/$id"
  if [ -d "$dir" ] && [ -f "$dir/closed_loop_trace.jsonl" ]; then
    echo "skip existing $id"; continue
  fi
  echo "=== running $id ==="
  timeout 900 ros2 launch car_ros2 car_sim.launch.py \
    ...
    mppi_hard_guard_switch_margin:=1.0 \
    mppi_hard_guard_min_dwell:=5 \
    mppi_hard_guard_warm_hard_return:=true \
