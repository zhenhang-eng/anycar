#!/usr/bin/env python3
"""KNN full-action vs residual retrieval contrast (anchor hypothesis test).

Direct test of the hypothesis that estimating the FULL action a* transfers
better across episodes than estimating the residual da* = a* - a0. The prior
KNN residual baseline copied the nearest train state's normalized residual
(OOF -0.43); this contrast adds the paired arm that copies the nearest train
state's FULL action a* directly (no anchor, no sigma scaling). Same folds,
same block-equal IQR distance metric, same real-rollout recovery metric.

If KNN-a* recovers substantially better than KNN-residual, the full action is
the more transferable target, supporting a direct full-action parameterization.
"""
...
DEFAULT_OUTPUT = Path(
    "outputs/mppi_proposal/knn_fullaction_contrast_20260818_v1"
)
...
def main() -> None:
    ...
    arms = ("residual", "full_action")
    report = {"per_fold": {}}
    pooled_gain = {arm: {1: [], 3: []} for arm in arms}
        ...
        for arm in arms:
            ...
            for k in (1, 3):
                ...
                if arm == "residual":
                    centers = np.clip(
                        a0_flat[test_rows]
                        + normalized_label[neighbor_rows].mean(axis=1)
                        * sigma_tiled[test_rows],
                        low, high,
                    )
                else:  # full_action: copy the neighbor's full action a*
                    centers = np.clip(
                        label_knots[neighbor_rows].mean(axis=1), low, high
                    )
                ...
                pooled_gain[arm][k].extend(gain.tolist())
            fold_result[arm] = arm_gain
    ...
    for arm in arms:
        pooled[arm] = {}
        for k in (1, 3):
            gain = np.asarray(pooled_gain[arm][k])
            pooled[arm][str(k)] = {
    ...
    summary = {
        ...
        "qualification": "KNN_FULLACTION_CONTRAST_DIAGNOSIS_ACTOR_FROZEN",
        "sources": {
        ...
        "protocol": {
            "states": count,
            "folds": args.folds,
            "split": "own deterministic speed-stratified episode folds",
            "metric": "block-equal IQR over six/reference/control/a0, train-fold fitted",
            "arms": {
                "residual": "copy k nearest train states' normalized residual, apply to query a0",
                "full_action": "copy k nearest train states' full action a* directly",
            },
        },
        "per_fold": report["per_fold"],
        "pooled": pooled,
        "nearest_distance_median_overall": float(
    ...
    print(json.dumps({
        "pooled": pooled,
        "nearest_distance_median": summary["nearest_distance_median_overall"],
    }, indent=1))
