#!/usr/bin/env python3
"""Qualify the K4/K16 90-round curve from its full-replay validator report."""
...
DEFAULT_OUTPUT = REPO_ROOT / "outputs/query_mppi/query_oac_gamma1_k4_k16_90round_20260902_v1"
...
def main() -> None:
    ...
    prior_curve = Path(config["sources"]["k1_k4_long_curve"])
    prior_manifest = json.loads((prior_curve / "manifest.json").read_text())
    ...
    expected_arms = [
        ...
        {"name": "gamma1_k16", "actor_updates_per_round": 16},
    ]
    checks = {
        ...
        "prior_k1_k4_curve_qualified": prior_manifest["qualification"] == "QUERY_OAC_GAMMA1_K1_K4_90ROUND_INDEPENDENT_PASS",
    ...
    qualification = (
        ...
        if passed else "QUERY_OAC_GAMMA1_K4_K16_90ROUND_INDEPENDENT_FAIL"
    )
    report = {
    ...
    if passed:
        ...
        manifest["prior_k1_k4_curve_manifest_sha256"] = sha256(prior_curve / "manifest.json")
