#!/usr/bin/env python3
"""Balanced hard-state g0 grouped CV (review fix of the first g0 round).

Changes vs run_mppi_g0_grouped_cv.py:
- Equal update weight: each epoch draws the same number of batches from the
  base internal-fit pool, the easy-fresh pool, and the hard pool (1:1:1).
  Inside the hard pool, strata are sampled uniformly (G0_ONLY gets the same
  weight as H_ONLY/JOINT), then the state, then the frame/targeted center.
- Control states are excluded from every training pool, including the
  targeted centers (6019 no longer trains its control state).
- Checkpoint selection uses easy internal-validation AND a grouped
  hard-validation split (20% of train-fold hard states, stratified): the
  score is the sum of both, and eligibility requires both norm ratios in
  [0.5, 2.0].
- The per-fold gate now includes the pre-registered P10 condition.
Same arms (PA_G0 / PA_G0_X), same deterministic fold assignment, 3 seeds.
"""
...
def center_metrics(model, inputs, context, target_gradient, batch_size, device):
    from run_mppi_g0_grouped_cv import center_metrics as _center_metrics
    return _center_metrics(model, inputs, context, target_gradient, batch_size, device)


def build_model(arm, args, device):
...
def g0_optimizer(model, args):
    from run_mppi_g0_grouped_cv import g0_optimizer as _g0_optimizer
    return _g0_optimizer(model, args)


def score_of(metrics: dict[str, float]) -> float:
    return (
        1.0 - metrics["cosine_median"]
        + 0.5 * (1.0 - metrics["cosine_p10"])
        + 0.2 * abs(np.log(metrics["norm_ratio_median"] + 1e-12))
    )


def sample_hard_batch(
    hard_states_by_stratum: dict[str, list[str]],
    items_by_state: dict[str, dict[str, np.ndarray]],
    strata: tuple[str, ...],
    batch_size: int,
    rng: np.random.Generator,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Stratum-uniform -> state-uniform -> item-uniform batch."""
    available = [s for s in strata if hard_states_by_stratum.get(s)]
    contexts, gradients, values, is_center = [], [], [], []
    for _ in range(batch_size):
        stratum = available[rng.integers(len(available))]
        state = hard_states_by_stratum[stratum][
            rng.integers(len(hard_states_by_stratum[stratum]))
        ]
        items = items_by_state[state]
        total = len(items["context"])
        index = int(rng.integers(total))
        contexts.append(items["context"][index])
        gradients.append(items["gradient"][index])
        values.append(items["value"][index])
        is_center.append(items["is_center"][index])
    return (
        np.asarray(contexts, np.int64),
        np.asarray(gradients, np.float32),
        np.asarray(values, np.float32),
        np.asarray(is_center, bool),
    )


def train_balanced_fold(
    arm: str,
    fold: int,
    seed: int,
    actor,
    ...
    easy_fresh: dict[str, np.ndarray],
    hard_states_by_stratum: dict[str, list[str]],
    items_by_state: dict[str, dict[str, np.ndarray]],
    hard_val_context: np.ndarray,
    hard_val_gradient: np.ndarray,
    scales: dict[str, Any],
    ...
    easy_context = torch.from_numpy(easy_fresh["context_index"]).to(device)
    easy_gradient = torch.from_numpy(easy_fresh["gradient"]).to(device)
    ...
    history = []
    strata = tuple(sorted(hard_states_by_stratum))
    for epoch in range(1, args.epochs + 1):
        model.train()
        losses = {"base": [], "easy": [], "hard": []}
        for pool in ("base", "easy", "hard"):
            for _ in range(args.batches_per_pool):
                if pool == "base":
                    pos_np = rng.choice(
                        train_positions, size=args.batch_size, replace=False
                    )
                    ...
                    use_value = F.smooth_l1_loss(
                        (q0 - tv) / value_scale, torch.zeros_like(q0), beta=0.5
                    )
                elif pool == "easy":
                    index = torch.from_numpy(np.asarray(
                        rng.choice(len(easy_context), size=args.batch_size)
                    )).to(device)
                    context = easy_context[index]
                    _, g0, _ = batch_parameters(model, inputs, context)
                    tg = easy_gradient[index]
                    use_value = None
                else:
                    ctx_np, grad_np, value_np, is_center_np = sample_hard_batch(
                        hard_states_by_stratum, items_by_state, strata,
                        args.batch_size, rng,
                    )
                    context = torch.from_numpy(ctx_np).to(device)
                    q0, g0, _ = batch_parameters(model, inputs, context)
                    tg = torch.from_numpy(grad_np).to(device)
                    center_mask = torch.from_numpy(is_center_np).to(device)
                    if bool(center_mask.any()):
                        tv = torch.from_numpy(value_np).to(device)
                        use_value = (
                            F.smooth_l1_loss(
                                (q0 - tv)[center_mask] / value_scale,
                                torch.zeros_like(q0[center_mask]), beta=0.5,
                            )
                            * float(center_mask.float().mean())
                        )
                    else:
                        use_value = None
                ...
                if use_value is not None:
                    loss = loss + args.value_weight * use_value
                optimizer.zero_grad(set_to_none=True)
                loss.backward()
                torch.nn.utils.clip_grad_norm_(model.parameters(), 5.0)
                optimizer.step()
                losses[pool].append(float(loss.detach()))
        easy_validation = center_metrics(
            model, inputs, validation_context, validation_gradient,
            args.evaluation_batch_size, device,
        )
        hard_validation = center_metrics(
            model, inputs, hard_val_context, hard_val_gradient,
            args.evaluation_batch_size, device,
        )
        score = score_of(easy_validation) + score_of(hard_validation)
        if epoch >= args.scheduler_min_epoch:
            scheduler.step(score)
        eligible = (
            epoch >= args.selection_min_epoch
            and 0.5 <= easy_validation["norm_ratio_median"] <= 2.0
            and 0.5 <= hard_validation["norm_ratio_median"] <= 2.0
        )
        ...
        if epoch == 1 or epoch % 10 == 0:
            history.append({
                ...
                "score": float(score),
                "loss": {key: float(np.mean(value)) for key, value in losses.items()},
...
def main() -> None:
    ...
    # Identical deterministic fold assignment to the unbalanced run.
    ...
    hard_pool_rng = np.random.default_rng(args.fold_seed + 1)
    ...
    for arm in arms:
        for fold in range(args.folds):
            ...
            easy_mask = (
                ~hard_mask & ~control_state_mask
                & ~np.isin(fresh_state, sorted(heldout))
            )
            easy_fresh = {
                "context_index": fresh_context[easy_mask],
                "gradient": fresh["gradient"][easy_mask].astype(np.float32),
            }
            trainable_states = [
                key for key in sorted(hard_states)
                if key not in heldout and key not in control_states
            ]
            # Grouped hard-train / hard-val split, stratified by stratum.
            train_states: list[str] = []
            val_states: list[str] = []
            for stratum in HARD_STRATA:
                pool = [
                    key for key in trainable_states
                    if hard_states[key]["stratum_id"] == stratum
                ]
                order = hard_pool_rng.permutation(len(pool))
                val_count = int(round(len(pool) * args.hard_validation_state_fraction))
            ...
            for key in train_states:
                frame_list = hard_states[key]["frames"]
                contexts = [
                    fresh_context[frame] for frame in frame_list
                ]
                gradients = [
                    fresh["gradient"][frame] for frame in frame_list
                ]
                values = [0.0] * len(frame_list)
                centers = [False] * len(frame_list)
                for row in target_rows:
                    if state_of(targeted["context_index"][row]) == key:
                        contexts.append(targeted["context_index"][row])
                        gradients.append(
                            targeted["local_gradient"][row, 0]
                        )
                        values.append(
                            targeted["transformed_reward"][row, 0, 0]
                        )
                        centers.append(True)
                items_by_state[key] = {
                    "context": np.asarray(contexts, np.int64),
                    "gradient": np.asarray(gradients, np.float32),
                    "value": np.asarray(values, np.float32),
                    "is_center": np.asarray(centers, bool),
                }
            hard_states_by_stratum: dict[str, list[str]] = {}
            for key in train_states:
                hard_states_by_stratum.setdefault(
                    hard_states[key]["stratum_id"], []
                ).append(key)
            hard_val_frames = np.sort(np.concatenate([
                hard_states[key]["frames"] for key in val_states
            ])) if val_states else np.empty(0, np.int64)
            for seed in seeds:
                model, training = train_balanced_fold(
                    arm, fold, seed, actor, inputs, labels, train_positions,
                    validation_positions, easy_fresh, hard_states_by_stratum,
                    items_by_state, fresh_context[hard_val_frames],
                    fresh["gradient"][hard_val_frames].astype(np.float32),
                    scales, args, device,
                )
                checkpoint_path = (
                    args.output_dir / f"g0b_{arm}_fold{fold}_seed{seed}.pt"
                )
                torch.save({
                    "format_version": 1,
                    "model_class": (
                    ...
                    "contract": {
                        ...
                        "balanced_pools": "base:easy:hard 1:1:1 batches per epoch",
                        "hard_sampling": (
                            "stratum-uniform -> state-uniform -> item-uniform"
                        ),
                        "checkpoint_selection": (
                            "easy internal-validation + grouped hard-validation"
                        ),
                    },
                }, checkpoint_path)
                ...
                records.append({
                    ...
                    "easy_fresh_frame_count": int(easy_mask.sum()),
                    "hard_train_state_count": int(len(train_states)),
                    "hard_validation_state_count": int(len(val_states)),
                    "training": {
    ...
    qualification = (
        "G0_BALANCED_GROUPED_CV_PASS_COMBINED_RETRAIN_AUTHORIZED"
        if any_pass else "G0_BALANCED_GROUPED_CV_HARD_STATE_TRANSFER_FAIL"
    )
    summary = {
        "format_version": 1,
        ...
        "protocol": {
            ...
            "fold_assignment": "identical to unbalanced run (same seed/order)",
            "pools": (
                "base internal-fit / easy fresh / hard fresh+centers, "
                f"{args.batches_per_pool} batches each per epoch"
            ),
            "hard_sampling": "stratum-uniform -> state-uniform -> item-uniform",
            "hard_validation_state_fraction": args.hard_validation_state_fraction,
            "hessian": (
                "heads excluded from optimizer, stay zero; all H-dependent "
                "losses dropped"
            ),
            "control_eval": (
                "21 control-role frames reported via strict 20; control "
                "states excluded from every training pool including "
                "targeted centers"
            ),
            "gates": {
                "cosine_median": 0.70, "cosine_p10": 0.0,
                "norm_ratio": [0.5, 2.0],
                "fold": ">=2/3 seeds pass all three gates; >=4/5 folds",
        ...
        "decision": {
            **decision,
            "authorization_note": (
                ...
                "gates; Actor stays frozen; formal validation/test sealed."
            ),
        },
        "contract": {
            "actor_frozen": True,
